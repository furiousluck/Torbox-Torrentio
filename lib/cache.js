...
export function cacheWrapStream(id, method) {
  console.log('cacheWrapStream', id, method);
    const newMethod = async () => {
        const result = await method();
        result.map(stream => remoteCache.set(`${STREAM_KEY_PREFIX}:${stream.infoHash}`,stream, { ttl: STREAM_TTL }));
        if (result) {
          console.log('Stream cache set', result.length);
            return result;
        }
        return null;
    }
  let x = cacheWrap(remoteCache, `${STREAM_KEY_PREFIX}:${id}`, newMethod, {
    ttl: (streams) => streams.length ? STREAM_TTL : STREAM_EMPTY_TTL
  });
  console.log('cacheWrapStream', x.length);
  return x;
}
